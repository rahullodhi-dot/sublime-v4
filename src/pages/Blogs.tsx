...
const displayPosts: BlogPost[] = [
  ...
  {
    id: 4,
    title: "The Art Of Brewing: Perfect Cup Tips For Every Tea Lover",
    slug: "art-of-brewing",
    date: "July 09, 2025",
    views: 248,
    excerpt:
      "Learn simple brewing methods and steeping times to enjoy richer aroma and balanced taste.",
    image: blogImg1,
  },
  {
    id: 5,
    title: "Wellness Teas For Busy Lifestyles",
    slug: "wellness-teas-for-busy-lifestyles",
    date: "July 20, 2025",
    views: 176,
    excerpt: "Quick tea rituals to refresh your day.",
    image: blogImg2,
  },
  {
    id: 6,
    title: "Seasonal Infusions You Must Try",
    slug: "seasonal-infusions-you-must-try",
    date: "August 01, 2025",
    views: 194,
    excerpt: "Explore new flavours for every season.",
    image: blogImg3,
  },
];

const relatedTags = ["Dates", "Dryfruits", "Gifting", "Herbals", "Tea", "Wellness"];

const Blogs = () => {
  const leftPosts = displayPosts.slice(0, 4);

  return (
    <section className="bg-[#f1e4b0] py-10 sm:py-14 lg:py-20">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-8 lg:gap-10 items-start">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {leftPosts.map((post) => (
              <article
                key={post.id}
                className="snap-start flex-shrink-0
                           w-full h-[442px] sm:h-full
                  ...
                  <div className="text-xs flex justify-between items-center text-gray-600">
                    {post.date} <div>
                      <svg xmlns="http://www.w3.org/2000/svg" width="57" height="21" viewBox="0 0 57 21" fill="none">
                        <path d="M53.532 0.358398H2.84673C1.47301 0.358398 0.359375 1.47596 0.359375 2.85455V17.9257C0.359375 19.3043 1.47301 20.4219 2.84673 20.4219H53.532C54.9057 20.4219 56.0193 19.3043 56.0193 17.9257V2.85455C56.0193 1.47596 54.9057 0.358398 53.532 0.358398Z" fill="#F6F1E8" stroke="#9A7522" strokeWidth="0.7" />
                        ...
                        <path d="M11.2891 6.62207C15.7841 6.62207 17.8594 10.7864 17.8594 10.7864C17.8594 10.7864 15.7841 14.9508 11.2891 14.9508C6.79403 14.9508 4.71875 10.7864 4.71875 10.7864C4.71875 10.7864 6.79403 6.62207 11.2891 6.62207Z" stroke="white" strokeWidth="0.929" strokeLinejoin="round" />
                        <path d="M13.254 10.7868C13.2596 11.0571 13.2117 11.3256 13.1132 11.5771C13.0146 11.8286 12.8663 12.0575 12.6777 12.2506C12.49 12.4437 12.2647 12.5972 12.0169 12.7018C11.7681 12.8073 11.5016 12.8609 11.2332 12.8609C10.9638 12.8609 10.6972 12.8073 10.4485 12.7018C10.2007 12.5972 9.97537 12.4437 9.78765 12.2506C9.59899 12.0575 9.4507 11.8286 9.35214 11.5771C9.25359 11.3256 9.20575 11.0571 9.21138 10.7868C9.21138 10.2489 9.42443 9.73274 9.80363 9.35219C10.1828 8.97165 10.6972 8.75781 11.2332 8.75781C11.7691 8.75781 12.2835 8.97165 12.6627 9.35219C13.0419 9.73274 13.254 10.2489 13.254 10.7868Z" stroke="white" strokeWidth="0.929" strokeLinejoin="round" />
          ...
          <aside className="bg-[#FFF7EA] rounded-xl p-4 sm:p-6 lg:p-7 h-fit">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#9a7523] mb-5">
              Recent Article
            </h2>
            ...
            <div className="space-y-4">
              {displayPosts.map((post) => (
                <Link
                  key={`recent-${post.id}`}
                  to={`/blogs/${post.slug}`}
                  className="flex gap-3 p-3 rounded-lg border border-[#ead9b4] bg-white/70 hover:bg-white transition-colors"
                >
                  <img
                    src={post.image || blogImg1}
                    alt={post.title}
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-[#9a7523] line-clamp-2">
                      {post.title}
                    </h3>
                    <p className="text-xs text-gray-600">{post.views ?? 0} views</p>
                  </div>
                </Link>
              ))}
            </div>

            <h3 className="text-xl font-bold text-[#9a7523] mt-8 mb-4">Related Tags</h3>
            <div className="flex flex-wrap gap-3">
              {relatedTags.map((tag) => (
                <button
                  key={tag}
                  type="button"
                  className="px-4 py-2 rounded-md bg-gray-50 border  text-[#9a7523] text-sm font-medium hover:bg-[#9a7523] hover:text-white transition-colors"
                >
                  {tag}
                </button>
              ))}
